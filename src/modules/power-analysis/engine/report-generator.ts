...
export function generateMethodsText(state: StudyState, results: any): string {
  ...
  let text = `Sample size was calculated a priori using Power Analysis Studio (Research Desk Pro). `;
...
export function exportToTxt(state: StudyState, results: any) {
  ...
  const content = 
    ...
    `  Generated by Power Analysis Studio (Research Desk Pro)\n` +
...
export async function exportToPdf(state: StudyState, results: any, element?: HTMLElement | null) {
  ...
  doc.text('Calculation Report — Research Desk Pro', 20, 28);
  ...
  doc.text('Power Analysis Studio — Research Desk Pro', 20, 290);
