...
export function generateMethodsText(state: StudyState, results: any): string {
  ...
  let text = `Sample size was calculated a priori using Power Analysis Studio (Reseonix). `;
...
export function exportToTxt(state: StudyState, results: any) {
  ...
  const content = 
    ...
    `  Generated by Power Analysis Studio (Reseonix)\n` +
...
export async function exportToPdf(state: StudyState, results: any, element?: HTMLElement | null) {
  ...
  doc.text('Calculation Report — Reseonix', 20, 28);
  ...
  doc.text('Power Analysis Studio — Reseonix', 20, 290);
